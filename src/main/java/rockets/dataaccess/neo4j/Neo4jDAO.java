...
public class Neo4jDAO implements DAO {
    ...
    private <T extends Entity> T findExistingEntity(Entity entity, Class clazz) {
        ...
        } else if (clazz.equals(Gunrunner.class)){
            Gunrunner gunrunner = (Gunrunner) entity;
            filters.add(new Filter("speed", EQUALS, gunrunner.getSpeed()))
                    .and(new Filter("usage", EQUALS, gunrunner.getUsage()))
                    .and(new Filter("fuel", EQUALS, gunrunner.getFuel()))
                    .and(new Filter("shape", EQUALS, gunrunner.getShape()));
            collection = session.loadAll(Gunrunner.class, filters);
        }
